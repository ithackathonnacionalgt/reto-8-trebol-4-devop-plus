...
export const CONRED_LISTING_URL = "https://conred.gob.gt/noticias/";

export interface ConredSourceOptions {
  listingUrl?: string;
...
export class ConredNewsSource implements NewsSource {
  public readonly name = "CONRED";
  private readonly client: HttpClient;
  private readonly options: Required<Omit<ConredSourceOptions, "clock">> & { clock: Clock };

  public constructor(client: HttpClient, options: ConredSourceOptions) {
    this.client = client;
    this.options = {
      listingUrl: options.listingUrl ?? CONRED_LISTING_URL,
  ...
  public async fetchCandidates(): Promise<NewsCandidate[]> {
    ...
    const candidates = parseConredListing(html, extractedAt, this.options.maxCandidates);

    if (candidates.length === 0) {
      throw new SourceError("La página de CONRED no contiene boletines o noticias reconocibles");
    }
...
export function parseConredListing(
  html: string,
  extractedAt: string,
  maxCandidates = 30,
): NewsCandidate[] {
  ...
  for (const match of html.matchAll(articlePattern)) {
    ...
    const url = resolveConredUrl(titleLink.href);
...
function resolveConredUrl(value: string): string {
  try {
    return normalizeUrl(new URL(value, CONRED_LISTING_URL).toString());
  } catch (error) {
    throw new SourceError("CONRED devolvió un enlace de boletín inválido", { cause: error });
  }
}
