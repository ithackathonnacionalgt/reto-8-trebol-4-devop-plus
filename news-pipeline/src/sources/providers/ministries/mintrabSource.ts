...
export class MintrabNewsSource implements NewsSource {
  public readonly name = "MINTRAB";
  private readonly client: HttpClient;
  private readonly options: Required<Omit<MintrabSourceOptions, "clock">> & { clock: Clock };

  public constructor(client: HttpClient, options: MintrabSourceOptions) {
    this.client = client;
    this.options = {
      listingUrl: options.listingUrl ?? MINTRAB_LISTING_URL,
  ...
  public async fetchCandidates(): Promise<NewsCandidate[]> {
    ...
    const candidates = parseMintrabListing(html, extractedAt, this.options.maxCandidates);

    if (candidates.length === 0) {
      throw new SourceError(
        "La página de MINTRAB no contiene convocatorias o noticias reconocibles",
      );
    }
...
export function parseMintrabListing(
  html: string,
  extractedAt: string,
  maxCandidates = 30,
): NewsCandidate[] {
  ...
  for (const match of html.matchAll(articlePattern)) {
    ...
    const url = resolveMintrabUrl(titleLink.href);
...
function resolveMintrabUrl(value: string): string {
  try {
    return normalizeUrl(new URL(value, MINTRAB_LISTING_URL).toString());
  } catch (error) {
    throw new SourceError("MINTRAB devolvió un enlace de noticia inválido", { cause: error });
  }
}
